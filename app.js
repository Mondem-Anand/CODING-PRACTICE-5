...
const datebasePath = path.join(__dirname, 'moviesData.db')
let database = null
...
const initializeDbAndServer = async () => {
  try {
    ...
    app.listen(3000, () =>
      console.log('Sever Runing at http://localhost:3000/'),
    )
  } catch (error) {
...
const convertDirectorDbObjectResponseObject = dbObject => {
  return {
    directorId: dbObject.director_id,
...
app.get('/directors/', async (request, response) => {
  ...
  response.send(
    directorsArray.map(eachDirector =>
      convertDirectorDbObjectResponseObject(eachDirector),
    ),
  )
})
